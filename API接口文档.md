...
### 模块说明
AI辅助功能，包括猫咪饲养建议、头像生成和语录生成。当前为Mock模式，API Key留空，后续接入真实AI API时只需替换服务实现。
...
| 参数名 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| question | String | 是 | 用户的问题描述，如"猫咪不吃东西怎么办？" |
| catBreed | String | 否 | 猫咪品种，如"橘猫"、"英短"、"布偶猫"，帮助AI给出更精准的建议 |
| catAge | String | 否 | 猫咪年龄，如"2岁"、"6个月"、"3岁" |
...
```json
{
  ...
  "catBreed": "橘猫",
  "catAge": "2岁",
  "advice": "建议每天定时喂食，保持饮食规律，避免频繁更换猫粮品牌。如果猫咪食欲不振，可以尝试将猫粮稍微加热，增加食物的香味来吸引猫咪进食。"
}
```
...
| 参数名 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| prompt | String | 是 | 用户对头像的描述，如"一只戴着皇冠的橘猫"、"布偶猫穿着宇航服" |
| style | String | 否 | 头像风格，默认`cartoon`。可选值：`cute`(可爱风)、`realistic`(写实风)、`cartoon`(卡通风)、`pixel`(像素风)、`watercolor`(水彩风) |
...
```json
{
  "prompt": "一只戴着皇冠的橘猫",
  "style": "cartoon",
  "url": "https://example.com/ai/avatar/1234567890?style=cartoon"
}
```
